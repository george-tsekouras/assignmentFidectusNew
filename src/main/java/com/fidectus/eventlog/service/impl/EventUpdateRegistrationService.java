...
@Component
public class EventUpdateRegistrationService extends EventTypeService {
    ...
    public EventType getType() {
        return EventType.UPDATE_REGISTRATION;
    }
    ...
    protected void validate(EventLog eventLog) {
        EventLog event = eventLogService.getLastEventByUserId(eventLog.getUserId())
                .orElseThrow(() -> new IllegalArgumentException("You cannot update registration info of an unregister user"));
        if (event.getType() == EventType.DELETED)
            throw new IllegalArgumentException("You cannot update registration info an deleted user");
    }
